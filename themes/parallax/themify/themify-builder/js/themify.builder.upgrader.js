			...
			if(username != "" && password != ""){
				...
				$.post(
					...
					function(data){
						...
						if(data == 'true'){
							hideAlert();
							jQuery('#themify_update_form').append( '<input type="hidden" name="plugin" value="'+ _updater_el.data( 'plugin' ) +'" /><input type="hidden" name="package_url" value="'+ _updater_el.data( 'package_url' ) +'" />' ).submit();
						} else if(data == 'false') {
							hideAlert('error');	
							showLogin('error');
						}
